...
We now introduce the addition operation on natural numbers as a term $+$ of the function type $\mathbb{N} \to \mathbb{N} \to \mathbb{N}$. A small note on this type signature is in order: in type theory, the function arrow is right-associative, meaning this is actually $\mathbb{N} \to (\mathbb{N} \to \mathbb{N})$. This reflects a technique called \textit{currying}: instead of a single function taking two numbers, $+$ is a function that takes one number and returns a new function (the \doublequotes{add $n$} function), which then takes the second number.

This syntactic convention takes the ambiguity out of the function signature itself, though it is important to distinguish this from the \textit{associativity of the operator} ($(a+b)+c = a+(b+c)$), which is a deep property that must be proven (by the way, Lean interprets the non-canonical $a+b+c$ as $(a+b)+c$). In alignment with the propositions-as-types paradigm, the properties of addition correspond to the inhabitancy of certain identity types. Addition is characterized by two fundamental such rules:
\begin{itemize}
...
\begin{proposition}\label{prop_comm}
...
After commutativity, we prove associativity:

\begin{proposition}\label{prop_assoc}
  The type $(a+b)+c = a+(b+c)$ is inhabited for all $a, b, c \in \mathbb{N}$.
\end{proposition}
\begin{proof}
  We shall prove this by induction on $c$. Base case: $c = 0$, meaning we wish to prove $(a+b)+0 = a+(b+0)$ is inhabited. By $\texttt{add\_zero}$, type $b+0=b$ is inhabited. Thus, we can rewrite the type $(a+b)+0 = a+(b+0)$ as $(a+b)+0 = a+b$. But by $\texttt{add\_zero}$ again, type $(a+b)+0 = a+b$ is inhabited. Thus, we can rewrite $(a+b)+0 = a+b$ as $a+b = a+b$. The inhabitant $\texttt{refl}_{a+b}$ concludes the proof of the base case. 
  
  Inductive step: Assume that the type $(a+b)+d = a+(b+d)$ is inhabited for some $d \in \mathbb{N}$. We wish to show that type $(a+b)+\text{succ}(d) = a+(b+\text{succ}(d))$ is inhabited. By $\texttt{add\_succ}$, type $(a+b)+\text{succ}(d) = \text{succ}((a+b)+d)$ is inhabited. Thus, we can rewrite the type $(a+b)+\text{succ}(d) = a+(b+\text{succ}(d))$ as $\text{succ}((a+b)+d) = a+(b+\text{succ}(d))$. Now, by $\texttt{add\_succ}$ again, type $b+\text{succ}(d) = \text{succ}(b+d)$ is inhabited. Thus, we can rewrite $\text{succ}((a+b)+d) = a+(b+\text{succ}(d))$ as $\text{succ}((a+b)+d) = a+\text{succ}(b+d)$. Now, by $\texttt{add\_succ}$ again, type $a+\text{succ}(b+d) = \text{succ}(a+(b+d))$ is inhabited. Thus, we can rewrite $\text{succ}((a+b)+d) = a+\text{succ}(b+d)$ as $\text{succ}((a+b)+d) = \text{succ}(a+(b+d))$. Now, by the inductive hypothesis (the inhabitant of $(a+b)+d = a+(b+d)$), we can rewrite the type $\text{succ}((a+b)+d) = \text{succ}(a+(b+d))$ as $\text{succ}(a+(b+d)) = \text{succ}(a+(b+d))$. The inhabitant $\text{refl}_{\text{succ}(a+(b+d))}$ concludes the proof. 
\end{proof}

The next proposition is a consequence of both commutativity and associativity:

\begin{proposition}
  The type $a + b + c = a + c + b$ is inhabited for all $a,b,c \in \mathbb{N}$.
\end{proposition}
\begin{proof}
By Proposition \ref{prop_assoc}, type $(a+b)+c = a+(b+c)$ is inhabited. Thus, we can rewrite the type $a+b+c = a+c+b$ as $a+(b+c) = a+c+b$. Once again by Proposition \ref{prop_assoc}, type $(a+c)+b = a+(c+b)$ is inhabited. Thus, we can rewrite the type $a+(b+c) = a+c+b$ as $a+(b+c) = a+(c+b)$. Finally, by Proposition \ref{prop_comm} the type $b + c = c+b$ is inhabited. Thus, we can rewrite the type $a+(b+c) = a+(c+b)$ as $a+(b+c) = a+(b+c)$. The inhabitant $\text{refl}_{a+(b+c)}$ concludes the proof.
\end{proof}
